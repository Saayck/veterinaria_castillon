...
| `SamarImportadora.bak` | Base OLTP Samar | ⚠️ **falta copiar al repo** (está en `C:\Users\Public\sqlbak\`) |
| `DW_SamarImportadora.bak` | Data Warehouse Samar | ⚠️ **falta copiar al repo** (está en `C:\Users\Public\sqlbak\`) |

> **Acción pendiente:** copia los 2 `.bak` dentro del proyecto para empaquetarlos:
> ```powershell
> mkdir sql-init\backups
> copy C:\Users\Public\sqlbak\SamarImportadora.bak      sql-init\backups\
> copy C:\Users\Public\sqlbak\DW_SamarImportadora.bak   sql-init\backups\
> ```
> (No los subas a GitHub si son pesados; sí van en el `.rar` de entrega.)

**Orden de restauración** (ya automatizado en `sql-init/setup-db.ps1`):
crear BDs → 3 dumps → `migracion-dump.sql` → restaurar los 2 `.bak`.
Detalle en `sql-init/README.md`.

---

## 3. Dos estrategias de despliegue (elige una)

### Estrategia A — BDs locales + Cloudflare Tunnel  ← más simple, recomendada
Las 5 BDs y la app corren en **tu equipo**; Cloudflare Tunnel les da un **link público** sin
abrir puertos. Cumple 10.1 (2 software con link) y mantiene las BDs locales.
...
## 4. Estrategia A — Cloudflare Tunnel (gratis, sin abrir puertos)

1. Levanta el sistema en Docker (BDs ya restauradas en tu SQL Server local, TCP 1433):
   ```powershell
   cd sql-init
   .\setup-db.ps1 -Server "localhost,1433" -User sa -Password "TuPassword"   # una vez
   cd ..
   docker compose up -d --build
   ```
   Verifica: http://localhost:5173 (app) y http://localhost:8080 (backend).
...
3. Publica (Quick Tunnel, sin cuenta ni dominio — la URL cambia en cada arranque):
   ```powershell
   # Terminal 1 — link del sistema (frontend; nginx ya enruta /api al backend)
   cloudflared tunnel --url http://localhost:5173
   # Terminal 2 — link del backend/API (para tener 2 software con link)
   cloudflared tunnel --url http://localhost:8080
   ```
   - **Link 1 (app):** `https://<algo>.trycloudflare.com`
   - **Link 2 (API):** `https://<otro>.trycloudflare.com/api/health`

   > Sin problemas de CORS: el frontend llama a `/api` **relativo** y nginx lo reenvía al
   > backend dentro de Docker; el navegador nunca cruza de dominio.

4. **URLs fijas (opcional, requiere dominio en Cloudflare):** usa un *Named Tunnel*
